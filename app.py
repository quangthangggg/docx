...
def process_document_xml(xml_path):
    ...
    logger.info("Bước 1: Đánh dấu các BLOCK_START0 cần xóa")
    block_para_ids = mark_paragraphs_for_removal(body, 'BLOCK_START', 'BLOCK_END', '0')
    logger.info(f"  Tìm thấy {len(block_para_ids)} paragraphs trong BLOCK_START0")

    logger.info("Bước 2: Đánh dấu các SECTION_START0 cần xóa")
    section_para_ids = mark_paragraphs_for_removal(body, 'SECTION_START', 'SECTION_END', '0')
    logger.info(f"  Tìm thấy {len(section_para_ids)} paragraphs trong SECTION_START0")

    logger.info("Bước 3: Xóa các paragraphs đã đánh dấu")
    ...
    logger.info("Bước 4: Xóa các table row có ROW0")
